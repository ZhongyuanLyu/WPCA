...
build_fred_rank_diagnostic <- function(data_dir) {
  rows <- lapply(c("FRED-MD", "FRED-QD"), function(dataset) {
    panel <- prepare_fred_panel(
      data_dir = data_dir, dataset = dataset, transform = FALSE
    )
    X <- panel$X
    R <- as.integer(floor(nrow(X) / 2))
    data.frame(
      Dataset = dataset,
      N = nrow(X),
      T = ncol(X),
      R = R,
      EstimatedRank = ratio_based_r(X, R = R),
...
one_fred_sim <- function(X, train_ratio, r) {
  ...
  # Inner CV must stay inside the outer training mask and account for both
  # retention probabilities when reconstructing its held-out entries.
  res_APCA <- CV_APCA(
    X_train, r, p_star = 0.8, J = 10,
    observed_mask = train_mat,
    base_observation_probability = train_ratio
  )
