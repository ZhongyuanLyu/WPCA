generate_signal_panel <- function(N, Times, r = 3L, phi_level = 0.9) {
  B0 <- matrix(0, nrow = r)
  Phi <- diag(phi_level, nrow = r, ncol = r)
  sigvare <- diag(1, nrow = r, ncol = r) - Phi %*% t(Phi)
  Fmat <- VAR1.sim(B0, Phi, Times, sigvare)
  Fmat <- stats::ts(Fmat)

  svd_L <- svd(matrix(stats::rnorm(N * r), nrow = N))
  L <- 1.2 * svd_L$u[, seq_len(r), drop = FALSE] %*%
    diag(svd_L$d[seq_len(r)], nrow = r, ncol = r)
  M <- L %*% t(Fmat)
...
symmetric_matrix_invsqrt <- function(A) {
  ...
  eig$vectors %*%
    diag(1 / sqrt(eig$values), nrow = nrow(A), ncol = ncol(A)) %*%
    t(eig$vectors)
}
...
draw_sigma_c <- function(N, rho = 0.6, covariance = c("offdiag", "diagonal", "isotropic")) {
  ...
  if (covariance == "isotropic") {
    return(10 * diag(1, nrow = N, ncol = N))
  }
  if (covariance == "diagonal") {
    variances <- stats::runif(N, 1, 20)
    return(diag(variances, nrow = N, ncol = N))
  }
  variances <- stats::runif(N, 1, 20)
  diag(variances, nrow = N, ncol = N) +
    rho * (rep(1, N) %*% t(rep(1, N)) - diag(1, nrow = N, ncol = N))
}
...
simulate_loading_distribution <- function(N, Times, rho = 0.6, gamma = 0, N_rep = 500L, r = 3L) {
  ...
  for (ii in seq_len(N_rep)) {
    ...
    LRes <- svd_nUUtX$u %*%
      diag(svd_nUUtX$d[seq_len(r)], nrow = r, ncol = r) -
      signal$L %*% rotation_cov$R_LQ
